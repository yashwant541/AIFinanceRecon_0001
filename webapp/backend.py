...
def _send_xlsx(data: bytes, filename: str):
    """send_file that works on both Flask 1.x (attachment_filename) and 2.x+
    (download_name)."""
    buf = io.BytesIO(data)
    try:
...
def _filter_tables(extractions, included):
    """Keep only the tables whose ids are selected (None/empty = keep all)."""
    if not included:
        return extractions
    keep = set(included)
    from financial_reconciliation.models.documents import (ExtractionResult,
                                                          FinancialDocument)
    out = []
    for e in extractions:
        tables = [t for t in e.document.tables
                  if api.table_id(e.document.filename, t.name) in keep]
        if not tables:
            continue
        doc = FinancialDocument(e.document.filename, e.document.file_format,
                                tables, dict(e.document.metadata))
        out.append(ExtractionResult(doc, list(e.warnings), {}))
...
def suggest_mapping():
    ...
    left = _filter_tables(list(s.left_extractions), body.get("included_left"))
    right = _filter_tables(list(s.right_extractions), body.get("included_right"))
    fields = api.suggest_mapping(left, right)
    left_profiles = [p.as_dict() for p in api.profile_side(left)]
    right_profiles = [p.as_dict() for p in api.profile_side(right)]
...
def export():
    ...
    try:
        ...
        return jsonify({"error": f"could not build the workbook: {e}"}), 400
    return _send_xlsx(data, "reconciliation.xlsx")


@app.route("/stage", methods=["POST"])
def stage():
    """Step 1 of upload: inspect files so the user can pick sheets/pages."""
    sid = request.form.get("sid", "")
    try:
        store = _get(sid)
    except KeyError as e:
    ...
    if not files:
        return jsonify({"error": "no files provided"}), 400
    staged = store.setdefault("staged", {})
    out = []
    for fs in files:
        data = fs.read()
        if len(data) > MAX_UPLOAD_MB * 1024 * 1024:
            return jsonify({"error": f"{fs.filename} exceeds {MAX_UPLOAD_MB} MB"}), 400
        info = api.inspect_file(fs.filename, data)
        key = uuid.uuid4().hex[:12]
        staged[key] = {"filename": fs.filename, "data": data}
        out.append({"staged_id": key, "filename": fs.filename, **info})
    return jsonify({"files": out, "engines": api.list_engines()})


@app.route("/upload_staged", methods=["POST"])
def upload_staged():
    """Step 2 of upload: parse staged files using the chosen sheets/pages."""
    body = request.get_json(force=True)
    try:
        store = _get(body.get("sid", ""))
    except KeyError as e:
    ...
    for sel in body.get("selections", []):
        entry = staged.get(sel.get("staged_id"))
        if not entry:
            continue
        sheets = sel.get("sheets") or None
        pages = api.parse_ranges(sel.get("pages"), int(sel.get("count") or 0))
        engine = sel.get("engine", "native")
        try:
            ext = api.extract_tables(entry["filename"], entry["data"],
                                     sheets=sheets, pages=pages, engine=engine)
        except Exception as e:  # noqa: BLE001
            return jsonify({"error": f"parse failed for {entry['filename']}: {e}"}), 400
        extractions.append(ext)
        warnings.extend([{**w.as_dict(), "file": ext.document.filename}
                         for w in ext.warnings])
        staged.pop(sel.get("staged_id"), None)   # free the bytes
...
def table_preview():
    """Return one table's columns + rows so the UI can display it."""
    body = request.get_json(force=True)
    try:
        store = _get(body.get("sid", ""))
    except KeyError as e:
    ...
    for e in exts:
        for t in e.document.tables:
            if api.table_id(e.document.filename, t.name) == tid:
                return jsonify({
                    "file": e.document.filename, "table": t.name,
                    "columns": t.columns, "total_rows": t.row_count,
                    "rows": [[r.values.get(c) for c in t.columns]
                             for r in t.records[:limit]],
                    "truncated": t.row_count > limit,
                    "qc": api.qc_table(t)})
...
def admin_reference_download():
    ...
    return _send_xlsx(data, "processed_tables.xlsx")
